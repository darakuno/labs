...
result = lambda i: (i >= max_i and True) or (char_list[i] == 'T') and print(calculate_triangle(input_data[i + 1][0], input_data[i + 1][1])) \
                   or (char_list[i] == 'C') and print(calculate_circle(input_data[i+1][0])) \
                   or (char_list[i] == 'P') and print(calculate_parallelogram(input_data[i+1][0], input_data[i+1][1]))
